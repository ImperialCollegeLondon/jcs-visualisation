% function [JCS_flex,JCS_ext,robot_pos_flex,robot_pos_ext,LVDT_flex,LVDT_ext] = tdms_extraction(data_in)
...
data_in=tdmsread('C:\Users\sholthof\OneDrive - Imperial College London\Imperial\PHD\SimVitro files\Cadaveric_MCL\SH48\004_Native_dynamic_lax_1BW\Data\004_Native_dynamic_lax_1BW_Neutral_flex_1of1_1_Main_processed.tdms');
    flex_ind = [];
    ext_ind = [];
    ap_flex=[];
    ap_ext=[];
...
for c = 1:numel(data_in)
        data = data_in{c};
 if sum(strcmp('LVDT', data_in{1, c}.Properties.VariableNames)) == 1
    LVDT_raw=data.LVDT;
 end
end

 LVDT_flex=[LVDT_flex,LVDT_raw(flex_ind)];
 LVDT_ext=[LVDT_ext,LVDT_raw(peakFlex+ext_ind-1)];
